(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('redux'), require('memoize-one'), require('immer'), require('redux-thunk'), require('react')) :
  typeof define === 'function' && define.amd ? define(['exports', 'redux', 'memoize-one', 'immer', 'redux-thunk', 'react'], factory) :
  (global = global || self, factory(global.EasyPeasy = {}, global.Redux, global.memoizeOne, global.immer, global.ReduxThunk, global.React));
}(this, function (exports, redux, memoizeOne, produce, thunk, React) { 'use strict';
  ...
  var createStore = function createStore(model) {
    ...
    var composeEnhancers = compose || (devTools && typeof window !== 'undefined' && window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ ? window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ : redux.compose);
    var store = redux.createStore(reducerEnhancer(reducers), defaultState, composeEnhancers(redux.applyMiddleware.apply(void 0, [thunk].concat(_toConsumableArray(middleware))))); // attach the action creators to dispatch
